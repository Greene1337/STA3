...
Tree * TreeCreate ( int _nNodes )
{
    ...
    memset( pTree->m_pNodeLabels, NULL, _nNodes * sizeof( char*));
...
char * CStringSetElementCopy( const char * _element )
{
	int length = strlen( _element );

	char * pResult = new char[ length + 1 ];

	strcpy( pResult, _element );
...
{
    assert( _nodeIndex < _tree.m_nNodes );
	delete [] _tree.m_pNodeLabels[ _nodeIndex ];
    _tree.m_pNodeLabels[ _nodeIndex ] = CStringSetElementCopy( _label );
}
